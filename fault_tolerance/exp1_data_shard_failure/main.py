...
class SparkFL:
    def __init__(self, spark, num_workers=2, num_rounds=100, batch_size=32, learning_rate=0.01):
        self.spark = spark
    ...
    def _save_accuracy_history(self):
        ...
        with open(self.accuracy_history_file, 'a') as f:
            for round_num, accuracy in self.accuracy_history:
                f.write(f"{round_num},{accuracy}\n")
        print(f"準確率歷史已保存到 {self.accuracy_history_file}")
