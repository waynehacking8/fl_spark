    ...
    def start(self):
        ...
        while True: # <<< 開始循環 >>>
            ...
            try:
                ...
                # 模擬participant節點故障（第5輪，participant-1）
                current_round = int(os.environ.get('FL_CURRENT_ROUND', '0'))
                if current_round == 5 and self.participant_id == 1:  # 第5輪，participant-1
                    logging.error(f"模擬participant節點故障：participant-{self.participant_id} 在第 {current_round} 輪離線")
                    # 直接關閉連接，模擬節點離線
                    client_socket.close()
                    break  # 退出循環，模擬節點徹底離線
